...
function onUrlChange() {
    setTimeout(function(){
        ...
        for (let u = 1; u < weigth1.length ; u++){
            let temp=weigth1[u].innerText.replaceAll(" ","").replaceAll("%","").replace("\n","").replace("\n","").replace("\n","").replace("\n","").replace("\n","").split("\n");
        ...
        chrome.storage.sync.get("gradeStyle",(res)=>{
            if(res.gradeStyle==true){
                for(i=points.length-1;i>-1;i--){
                    let pointst=points[i].innerHTML.replace("/ ","").replace(" pts","").split(" ")
                    let grade = pointst[0]
                    let max = pointst[1]
        ...
        chrome.storage.sync.get("grade",(res)=>{
            if (res.grade==true){
        ...
        chrome.storage.sync.get("finalColor",(res)=>{
            for(let l=0;l<tempt.length;l++){
                if(tempt[l].innerText=="Final Grade: "+gradeavgstr){
                    tempt[l].style.color=res.finalColor
                }else{}
            }
        })
    },2500)
